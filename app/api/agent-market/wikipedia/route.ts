...
export async function POST(request: NextRequest) {
  ...
  const { query, tool = 'wiki_search' } = body
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        sandbox = await Sandbox.create({ mcp: { wikipedia: {} } })

        send('status', 'Sandbox ready. Connecting to Wikipedia MCP server...')

        client = new Client({ name: 'wiki-mcp-client', version: '1.0.0' })
        ...
        const resolvedTool = tools.find((t) => t.name === tool || t.name.endsWith(`-${tool}`) || t.name.endsWith(`/${tool}`))
