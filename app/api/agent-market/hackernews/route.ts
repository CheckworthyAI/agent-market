...
export async function POST(request: NextRequest) {
  ...
  const { query, tool = 'hn_search' } = body
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        sandbox = await Sandbox.create({ mcp: { hackernews: {} } })

        send('status', 'Sandbox ready. Connecting to Hacker News MCP server...')

        client = new Client({ name: 'hn-mcp-client', version: '1.0.0' })
